...
async function seedDatabase() {
  ...
  const camp = await db.prepare('INSERT INTO campaigns (name, description) VALUES (?,?)').run(
    'Instituto Novo Horizonte', 'Programa Horizonte Mulher'
  );

  const rifa = await db.prepare(`
    INSERT INTO rifas (
  ...
  const numStmts = [];
  for (let i = 1; i <= qty; i++) {
    numStmts.push({ sql: 'INSERT INTO rifa_numeros (rifa_id, number) VALUES (?,?)', args: [rifaId, i] });
  }
  await db.runBatch(numStmts);
  ...
  for (let i = 0; i < 40; i++) {
    const p = await db.prepare('INSERT INTO participants (rifa_id, name, cpf, whatsapp, email, city, state) VALUES (?,?,?,?,?,?,?)').run(
  ...
  const paidNums = await db.prepare('SELECT id, number FROM rifa_numeros WHERE rifa_id=? AND id <= ?').all(rifaId, sold);
  ...
  const resNums = await db.prepare('SELECT id, number FROM rifa_numeros WHERE rifa_id=? AND id > ? AND id <= ?').all(rifaId, sold, sold + 5);
  for (let i = 0; i < resNums.length; i++) {
    const n = resNums[i];
    const pid = participants[0];
    const resCode = genCode('R', 6);
    const oIns = await db.prepare('INSERT INTO orders (rifa_id, participant_id, code, status, qty, unit_price, discount, total) VALUES (?,?,?,?,?,?,?,?)')
      .run(rifaId, pid, resCode, 'pending', 1, 10, 0, 10);
    const oid = oIns.lastInsertRowid;
    await db.runBatch([
      { sql: 'INSERT INTO order_numbers (order_id, numero_id, rifa_id, number) VALUES (?,?,?,?)', args: [oid, n.id, rifaId, n.number] },
